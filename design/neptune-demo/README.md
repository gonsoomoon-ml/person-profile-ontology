...
**💼 비즈니스 기능 요약 (이 데모가 주는 가치):**

- 🎯 **개인화 추천** (행동 취향 + 동네, 가본 곳 제외) → 재주문·전환 ↑
- 🛡️ **식이 제한 안전 추천** (채식·비건 위반 제외) · 💬 **설명 가능(why)** (최단 경로·meta-path)
- 👤 **고객 이해·세분화** (프로파일 카드·RFM·지출, Louvain 자동 세그먼트)
- 📣 **룩어라이크 확장** (Jaccard·공유 요리) · 🆕 **콜드스타트** · 🧺 **크로스셀·번들** (요리 동시출현·CF)
...
| **6) 말한 것 vs 실제**<br>(mismatch) | 🔧 선언 선호 `LIKES_CUISINE`와 실제 최다 주문 요리를 비교해 불일치 플래그<br>💡 '말 vs 행동' 격차 탐지 — 선언 신호를 행동으로 보정(프로파일 신호의 하나) | 📊 7명 일치, **Daniel=MISMATCH** (선언 이탈리안 / 실제 멕시칸)<br>→ 선언보다 행동을 신뢰 |
